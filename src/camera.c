...
int camera_configure(struct camera *camera, uint32_t format, uint16_t width, uint16_t height)
{
    ...
    for (int i = 0; i < num_planes; i++)
    {
        ...
        camera->config.sm_cfgs[i + 1] = camera_pio_get_shift_byte_sm_config(
            platform->pio, i + 1, camera->shift_byte_offset, platform->base_pin, xfer_bytes * 8);
    }
                           ...
                           bool allow_reconfigure, bool blocking)
{
    ...
    if ((camera->config.format != buf->format) || (camera->config.width != buf->width) ||
    ...
    for (int i = 0; i < num_planes; i++)
    {
        dma_channel_configure(camera->dma_channels[i], &camera->config.dma_cfgs[i], buf->data[i],
                              ((char *)&platform->pio->rxf[i + 1]) + camera->config.dma_offset[i],
                              camera->config.dma_transfers[i], true);
    }
