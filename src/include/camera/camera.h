...
//
// If the camera is already configured for the same format/width/height as
// 'into' then no reconfiguration happens. Otherwise, if allow_reconfigure is
// true then the camera will first be reconfigured, and if not, capture will
// fail.
//
// Returns 0 on success
